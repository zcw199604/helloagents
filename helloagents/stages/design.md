...
```yaml
前置: EVALUATE 阶段完成（评估确认：评分≥8 或 R2 已确认）
...
子代理调用:
  complex+涉及>5个独立单元 → 调度原生子代理执行深度依赖分析和质量评估（强制）[→ G10 调用通道]
    编排: 按涉及单元拆分，每个子代理负责分析一个单元（子代理数=单元数，≤6/批）
    每个子代理 prompt 明确: 负责的单元路径 + 分析内容（依赖关系/接口/质量问题/风险）
    主代理汇总各子代理结果，合并为完整技术分析报告
  其他 → 主代理直接执行

并行优化: 当 complex+涉及>5个独立单元 且步骤4和步骤6均需调用原生子代理时:
...
R3 标准流程（CRITICAL）:
  ...
  方案评估: 所有子代理返回后，主代理统一评估对比 → 为最优方案标记"推荐"（按 G4 推荐标记规则）
...
主体内容:
  ...
  （空行）
  方案对比（每个方案用 - 标记，非数字编号：名称、核心思路、成果设计方向、优缺点、推荐标记）
  （空行）
选项:
  选项：
