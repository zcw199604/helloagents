...
- `skills/helloagents/` 是唯一手工维护的 Skill 源；Codex 与 Claude 两端目录由同步脚本生成。
...
- collaborating bridge 以 `scripts/*_bridge.py` 为权威源，同步时写入 canonical Skill 资源并生成宿主副本。
...
- 修改 canonical Skill 或 bridge 后先运行 `python scripts/sync_skills.py --write`，CI 使用 `--check` 阻止分发漂移。
