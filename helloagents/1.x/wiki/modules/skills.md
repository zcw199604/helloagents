...
- 文件数量、主观需求评分和启用协作均不单独升级流程；边界已明确的整体清理按依赖顺序完成全部已授权范围。
...
`requires` 仅表示硬依赖。需求分析需要知识库专门规则时才读取 `kb`；知识库实际创建文件时才读取 `templates`，只读分析不预加载写作模板。
...
不传 `--results` 只校验用例数据，不代表模型行为已通过评估。用例覆盖批量机械调整、已知范围的整体清理、只读全量审查和单个业务缺口；需要判断实际行为时使用隔离的真实情景演练，不以规则中包含某句话代替行为验证。

审计通过表示:
- frontmatter 字段、类型和重复键合法。
- `references/*.md` 及普通 Markdown 文件链接存在。
- 保留的编号规则引用能解析到技能树中的规则标题；失效引用报告文件和行号，新规则优先链接具体文件与章节。
- `requires` 依赖图无环。
...
有方案包或用户明确要求结构化 QA 时，`qa-review` Skill 在步骤7质量检查与测试完成后、交付前读取。默认在方案包中写入 `qa-review.json`；用户单独要求结构化 QA 时可按指定位置或既有约定保存独立证据，不补建方案包。新记录使用 schema v3，并在 `tdd` 中保存分类、决策、目标行为和阶段结果；历史 v1/v2 记录继续可读。该文件记录:
- QA 模式、审查范围和结论。
- 实际运行的验证命令及结果。
- 阻断项、警告项和信息项。
...
自适应无方案包改动的 TDD 证据保留在最终摘要，不强制写 JSON。阻断项始终阻止归档和成功总结；修复仍在既有授权内时继续修复和验证，只有缺少必要决定、超出授权或需要接受 P1 风险时才询问。

### 完成声明与证据复用

`verification-before-completion` 统一判断验证证据有效性和结论范围。代码、依赖、相关环境未变化且证据可核实时，同一任务可以复用结果，恢复上下文本身不触发重跑。新变化或故障信号只使相关证据失效，按受影响范围补验；目标测试通过不能描述为全量通过。无需重构时，TDD 的 REFACTOR 记录可引用仍有效的 GREEN 结果，不为填充字段重复执行。
...
- 子代理派发可使用方案包，或已明确的请求目标、文件范围与验收标准；只读子任务写入范围为空，无方案包协作不补建流程文件。
...
先从相关代码和已有文档查证术语与业务规则；只有无法查证且会改变实现、验收或授权边界的缺口才追问，不固定问题数量。只读审查可直接调查并报告。稳定且会被未来任务复用的术语记录为候选，再按知识库写入收益门禁决定是否沉淀到 `wiki/glossary.md`。
