...
规则:
  ...
  3. 审查结果按严重程度分级
  4. 发现问题时提供优化方案选项
  5. 审查文件>5 时调度原生子代理并行执行各维度分析 [→ G10 调用通道]
  6. 审查文件>5 时 [RLM:reviewer] 强制调用 [→ G10 调用通道]
  7. 由 ~exec 调用时执行多模型一致性复核（方案包文档 ↔ 代码实现）
```
...
分析调度:
  审查文件≤5: 主代理串行执行各维度分析
  审查文件>5: 根据文件数量和复杂度，将上述维度分配给原生子代理并行执行（子代理数=维度数，≤6/批）[→ G10 调用通道]
    分配原则: 每个子代理负责≥1 个维度，维度间无数据依赖可自由分组
  所有子代理完成后: 主代理汇总 → 按严重程度分级（Critical/Warning/Info）
