...
当用户选择执行方式:
  ...
  交互式执行:
    ...
    按 R3 标准流程进入阶段链，关键决策点 ⛔ END_TURN（方案选择、失败处理）
...
```yaml
R3 标准流程阶段链:
  1. 按 G7 按需读取表"R2/R3 进入方案设计（入口）"加载 → 方案设计（含上下文收集+多方案对比）
  2. 按 G7 按需读取表"R2/R3 进入开发实施（入口）"加载 → 开发实施
  3. 验收: 按 G8 流程级验收规则执行
  4. 输出: 完成（验收报告+变更摘要+遗留提示）→ 状态重置
```
