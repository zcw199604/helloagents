...
- 同一需求因上下文压缩、会话恢复或长任务续接而继续时，不生成新的 `WORKFLOW_ID`，也不把恢复动作视为新的分析、设计或开发阶段。
- 优先从 `CURRENT_PACKAGE`、`task.md` 状态、未完成 TODO、最近变更清单和已有验证证据恢复；仅读取确认当前进度、阻塞项和下一步所需的最小上下文。
- 已标记完成且证据仍有效的任务、分析、审查和验证不得自动重放；未被本轮触达的既有代码不得因恢复而扩大审查。
- 仅当代码或依赖已变化、证据缺失或失效、出现新的风险/故障信号，或用户明确要求时，才重新检查对应范围。
- 状态或证据无法确认时，执行有边界的只读探查并记录不确定性；不得凭空推断任务已完成，也不得因此重扫整个项目。
...
- 遗留扫描已排除本次创建或执行的方案包。
- 发生上下文恢复时，已复用仍有效的任务状态和验证证据，未无故重放已完成工作或扩大审查范围。
