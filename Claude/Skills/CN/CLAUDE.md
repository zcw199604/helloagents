...
- 动作:
  ...
  5. 连续3次修复失败或无法定位根因 → 暂停扩大改动，重新进入需求分析/方案设计或按G3请求确认
  6. 根因明确后，按影响范围进入微调/轻量迭代/标准开发/完整研发；若涉及EHRB则按完整研发处理
- 输出:
  - 需要用户补充复现信息时: 按需求分析追问格式
  - 已完成修复时: 按最终实际开发模式输出（微调/轻量迭代/开发实施/命令完成）
...
**关键步骤:**
- 方案构思: 知识库检查、项目规模判定、任务复杂度判定、大范围方案拆分、方案构思
- 详细规划: 创建方案包目录、生成 why.md/how.md/task.md、设计边界检查、风险规避、方案自审门禁
...
2. 检查知识库状态
3. 读取方案包
4. 并行子代理适配判断（适用时读取 `hello-subagent` Skill）
5. 系统化调试门禁（Bug/测试/构建/运行异常适用）
6. 按任务清单执行代码改动
7. 大型项目最小改动执行约束（适用时）
8. 代码安全检查
9. TDD门禁与质量检查测试（适用时读取 `tdd` Skill）
10. 同步更新知识库
11. 更新 CHANGELOG.md
12. 一致性审计
13. 代码质量检查（可选）
...
| 系统化调试 | `develop` | 检测到Bug、测试失败、构建失败、运行异常、性能回退或flaky时读取 |
| 测试驱动开发 | `tdd` | 新功能、Bug修复、行为变更、核心逻辑调整或测试策略设计时读取 |
| 并行子代理编排 | `hello-subagent` | 开发实施或多模型协作中存在多个独立、边界清晰、可验证的子任务时读取 |
